...
This sample is very simular to [Update a Bug](#samples-updateabug) example. The difference is, we only need to add a value for the System.History field.
...
The below example is very simular to the [Update a Bug](#samples-updateabug) example. You just need to set the ```/fields/System.WorkItemType``` field to a User Story.
