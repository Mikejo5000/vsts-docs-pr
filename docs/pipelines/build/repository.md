...
> [!NOTE]
> If people push changes frequently to the repository that you want to leverage shallow fetch in your build definition, you might want to increase the fetch depth to a reasonable value. Your build may fail when your fetch depth is too small and there are changes pushed to the branch you are trying to build between build get queued and agent pick up the job.
