...
-   Each server that is running Release Management Server, or any serversÂ¹ in a release environment that are running the deployment agent, if Release Management is part of your deployment.
...
You can deploy Release Management with HTTPS completely separate from TFS, regardless of the protocol youâ€™re using for TFS, or if youâ€™re using TFS at all. However you decide to deploy Release Management, the instructions for creating a secure deployment for Release Management are very similar to whatâ€™s set down here for TFS. The big difference is the procedure for binding the HTTPS protocol to the Release Management website, which is covered below.

To deploy Release Management with HTTPS, use the task list below. If youâ€™re configuring Release Management with TFS, skip over any tasks you might have already completed for TFS configuration.
...
4.  In the **Configure Services** dialog box, select HTTPS and enter the fully qualified domain name and the SSL port of the Release Management Server, and then choose **OK**. Youâ€™ll be prompted to restart the application.
...
6.  Repeat these steps for the account of every user who has accessed Team Foundation from that computer.
    ...
    > [!NOTE]
    > You might want to distribute instructions for clearing the cache to all of your Team FoundationÂ users so that they can clear the caches for themselves.
