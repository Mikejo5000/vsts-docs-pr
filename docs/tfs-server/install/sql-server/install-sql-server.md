...
-   TFS reporting is optional, but if you want reporting you must install Reporting Services and Analysis Services (each can go on its own server, if you want that topology). If you skip reporting, you donâ€™t have to set up Reporting Services or Analysis Services.  
-   If you donâ€™t install any SQL Server instances on the TFS server, you must at least install the SQL Server feature Client Tools Connectivity on TFS. If you have a SQL Server instance on the TFS server (either the Database Engine, Reporting Services or Analysis Services) you donâ€™t have to install Client Tools Connectivity.
...
6.  On the **Setup** **Support Rules** page, choose **Next**.
    ...
    > [!TIP]
    > A Windows Firewall warning might appear, but you can safely ignore this warning if youâ€™re planning to also install TFS on this server. TFS automatically adds an exception to Windows Firewall for SQL Server, if both servers (TFS and SQL Server) are installed on the same machine. If youâ€™re installing TFS on some other server, youâ€™ll want to [open a port for SQL Server in Windows Firewall on this server](http://elhajj.wordpress.com/2013/02/25/workaround-error-tf255049-punching-a-hole-through-windows-firewall/) (blog post).
	>
...
8.  On the **Feature Selection** page, select the check boxes for one or more of the following components, depending on the topology you intend to use:
    ...
    -   Reporting Services â€“ Native (required for reporting)
    ...
    -   Management Tools - Basic Â¹ (not pictured)
...
Â¹ To install Team Foundation Server,Â you do not need to install **Management Tools**Â (Basic or Complete) on the same computer asÂ SQL Server. However, you must use the SQL Server Management Studio management tool to verify your installation of SQL Server.
...
If you donâ€™t install the Database Engine on the same server as Reporting Services, you must manually configure your report server using the steps below after you finish SQL Server installation.Â 
