...
4. Enter the URL in the format below and click OK:  
	...
	If the *account* is "fabrikam" then the URL is ```https://fabrikam.analytics.visualstudio.com/_odata```.

	>[!NOTE]  
	>Alternatively, you can enter the URL ```https://{account}.analytics.visualstudio.com/ProjectName/_odata``` which will trim the results by the specified team project across all entities related to that project.  
