...
4. Enter the URL in the format below and click OK:  
	...
	```
	https://AccountName.analytics.visualstudio.com/_odata
	```  

	![Select OData Feed ](_img/pbi3.png)  

	If the *AccountName* is "fabrikam" then the URL is ```https://fabrikam.analytics.visualstudio.com/_odata```.

	>[!NOTE]  
	>Alternatively, you can enter the URL ```https://[collection name].analytics.visualstudio.com/ProjectName/_odata``` which will trim the results by the specified team project across all entities related to that project.  
