...
<tr>
	...
	<td><p>Forces the file content clean-up process to start immediately after the content deletion finishes. If the user does not specify <strong>/startcleanup</strong>, the destroyed content clean-up process occurs when the database maintenance cleans up all the files that are no longer referenced by Visual Studio Team Foundation Server. By default, the clean-up is scheduled to run every 5 days.</p></td></tr>
<tr>
