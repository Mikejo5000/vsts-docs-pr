...
-   Click **Start**, click **All Programs**, click **Microsoft Visual Studio 2008**, click **Visual Studio Tools**, and then click **Visual Studio Command Prompt**.
    ...
    -   Use the **/startcleanup** option to immediately clean up the metadata of the files that are no longer referenced by Team Foundation Server. Without this option, the metadata of the destroyed files are removed when the database is maintained by a SQL process that runs every 5 days. Seven days after the metadata deletion, the content of the destroy files will be deleted by another SQL process.
