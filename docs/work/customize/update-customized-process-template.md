...
> [!IMPORTANT]  
>This topic applies only to projects defined on an on-premises Team Foundation Server (TFS). Projects hosted on Azure Codex [update automatically with each service upgrade](/vsts/release-notes/index). To update a project that uses the Hosted XML process, see [Import process](import-process/import-process.md).
...
1. [Upload the process template](../work-items/guidance/manage-process-templates.md).
	...
	To verify your changes, create a test project. See [create a project](../../accounts/create-team-project.md).
