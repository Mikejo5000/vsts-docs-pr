...
> | Default properties | The default properties set for teams that you add to a project are preserved as documented in [Process configuration XML element reference, Specify properties and behaviors](../reference/process-configuration-xml-element.md#specify-properties-and-behaviors). | 
...
> | Four column layout  and size | The inherited process supports a [a fixed relative sizing of three columns](../../../organizations/settings/work/inheritance-process-model.md#resizing) to a WIT layout, while the Hosted XML process supports up to four columns and allows you to set the first column as equal sized with the rest of the columns.  |
...
> |Categories | Changes made to a [default category](../reference/use-categories-to-group-work-item-types.md#default-categories-and-process-configuration) are preserved, but any custom categories are ignored. | 
