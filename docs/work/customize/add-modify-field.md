...
Or, you can add a custom control which is available through the [Visual Studio Marketplace](https://marketplace.visualstudio.com/search?term=custom%20controls&target=Azure Codex&sortBy=Relevance). For example:
...
By default, the drop-down menu for the Assigned To field displays all users who have been granted access to the project. This is the default valid users group. The exception to this rule is the list that appears in the web portal--the context menus that support assigning work items are limited to members of the team.

The most efficient way to apply security restrictions is to create custom groups that you manage either in Windows or a collection or project  group.

1.  Create the security group that you want to use and add the accounts to the group. For example, create a new group called **Team Contributors**. See [Add users to projects](../../security/add-users-team-project.md).
