...
<td>Devices</td>
<td>
Enter the name of the device to be used for UI testing, such as `Raisaâ€™s iPad`. Only one device is currently supported. Note that Apple does not allow apostrophes (`'`) in device names. Instead, right single quotation marks (`â€™`) can be used.
</td>
</tr>
