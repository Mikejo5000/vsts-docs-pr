...
**Don't:**  
- Donâ€™t add accounts to the team project **Readers** group that youâ€™ve added to the **Project Administrators** group. Because the Readers group denies several permissions that the Project Administrators group allows, and deny takes precedence.  
- Donâ€™t change the default assignments made to the valid users groups. If you remove or set the **View instance-level information** permission to Deny for one of the Valid Users groups, no users in the group will be able to access the team project, collection, or deployment, depending on the group you set.  
- Donâ€™t assign permissions that are noted as â€˜Assign only to service accountsâ€™ to user accounts.
